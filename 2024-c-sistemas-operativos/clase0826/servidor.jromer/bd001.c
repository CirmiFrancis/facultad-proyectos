...
void funcionPostgresql(const char *,const char *,const char *,const char *,char [],char [],char []);
...
int main(void) 
{
    ...
    const char * remote_host_postgres = REMOTEHOSTPostgres;
    const char * remote_port_postgres = REMOTEPORTPostgres ;
    ...
    char mensaje[8092]; 
    armarResultadoQuery(lineaDatos,mensaje,remote_host_postgres,remote_port_postgres);
    ...
    printf("<h1>Datos Recibidos de Postgresql</h1>");
    printf("<p>%s</p>\n", mensaje);
...
void armarResultadoQuery(char recibe[],char mensaje[], const char * host , const char * port)
{
      ...
      char SQL_respuesta_SQL[1024] ; memset(SQL_respuesta_SQL,'\0',1024) ;  
       ...
       //Postgresql
       //  strcat(mensaje,"SGBD --> Postgresql<br>");        
         if (VERBD[0] == 'S') //MOSTRAR BD
         {
           memset(SQL_consulta,'\0',1024);
           strcpy(SQL_consulta,"select datname from pg_database WHERE datistemplate =false;\0");
           consulta(host,
                    port,
                    USERNAME,
                    USERPASS,
         ...
         {
          memset(SQL_consulta,'\0',1024);
          strcpy(SQL_consulta,"select table_name from information_schema.tables WHERE table_schema = 'public';\0");
          consulta(host,
                     port,
                           USERNAME,
                           USERPASS,
         ...
         if (NAMETAB[0] != '\0' && VERATTR[0] == 'S') //MOSTRAR ATRIBUTOS DE LA TABLA
         {
          memset(SQL_consulta,'\0',1024);
          strcpy(SQL_consulta,"select attname from pg_catalog.pg_attribute inner join pg_catalog.pg_class on pg_catalog.pg_class.relfilenode = pg_catalog.pg_attribute.attrelid WHERE pg_catalog.pg_class.relname = '");
